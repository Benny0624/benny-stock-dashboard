...
**2026-09-07 加碼**：Equity Curve、Underwater Chart、交易標的價格圖三張
都補上 `dataZoom`（跟指標 dashboard 同一套 `{type:"inside"}` + slider），
在瀏覽器實測過，拖拉條正常顯示、能框選任意區間看細節。月度報酬熱力圖是
類別軸（年 x 月），不是時間序列，沒有加（拖拉縮放對熱力圖沒有意義）。
