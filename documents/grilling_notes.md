...
- benny-stock-dashboard/output/dashboard.html 
 1. 指標複選刻度會以誰的為主? 目前實測是以大的為主 這樣是不是要標明在座標軸?
 2. 時間範圍不能自由拉選，比如說我要兩年前往後3個月，現行儀表板做不到
 3. layer 3 html 都已經做到 10 年資料了，評估一下這邊顯示十年會不會負擔太大
 4. 想新增手機板本的閱讀模式

### 電手回覆（2026-09-06）

1. **座標軸不是「誰原始數值大」主導**——所有選取的指標都已經正規化成
   「窗口起點 = 100」再疊圖，不是原始尺度。你看到的「以大的為主」其實是
   「**波動幅度最大**的那條線把 y 軸範圍撐開」（例如 VIX 平常在 100 附近，
   恐慌時衝到 300+，同一張圖上其他線就被壓在底部一小段），這是「共用線性
   Y 軸比較多條正規化曲線」的固有取捨，不是 bug。**建議**：先在 hint 文字
   加一句說明（成本最低），如果之後常常被 VIX 這種極端值洗版，再考慮加
   log scale 切換或雙 y 軸——這兩個都是要新增 UI 控制項的中型改動，先問
   你要不要現在做還是先加文字說明就好。
2. **時間範圍改用 ECharts `dataZoom`（slider/縮放）取代自由日期輸入**：
   自己刻兩個 date input 要處理格式驗證、跟現有 `window-select` 邏輯打架；
   `dataZoom` 是圖表原生功能，直接在圖上拖拉/框選任意區間，比輸入日期更
   符合「看線找區間」的操作習慣，改動集中在圖表 option，不用動資料查詢
   邏輯（反正 2 年資料都已經在前端了，純前端縮放不用重查 DB）。
3. **10 年資料量評估**：這次 2 年窗口、14 個指標的 `dashboard.html` 實測
   約 900KB～1MB。10 年大概是資料列數的 5 倍，估計整份 html 會落在
   4-5MB——GitHub Pages 沒有這個量級的檔案大小問題，行動網路下載也還在
   可接受的幾秒等級，**不是檔案大小風險**。真正的取捨是**看圖體驗**：
   2 年窗口目前一條線約 500 個資料點，10 年會變 2500 點，前端全部渲染會
   讓 hover/縮放互動變頓（尤其手機）。**建議**：dashboard 顯示層維持
   2 年上限（決定 15 的理由不變——回測要長歷史、顯示不需要），真的想看
   更長期趨勢就靠上面第 2 點的 `dataZoom` 在既有資料上局部放大，不需要
   為此多抓 10 年資料進前端。
4. **手機閱讀模式**：維持先前拍板——排在 multi-select 疊圖引擎穩定之後，
